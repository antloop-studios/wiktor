...
function mktree(json, path = []) {
   ...
   if (Array.isArray(json)) {
      ...
      json.forEach(function (e) { mktree(e, path).appendTo(chunk) });

   } else if (typeof (json) === typeof ({})) {
      chunk = $('<ul></ul>');
      for (var key in json) {
         $("<a id='" + key + "'><li class='sep'>" + key + "</li></a>").appendTo(chunk);
         var inner = $('<ul></ul>');
         path.push(key);
      ...
      chunk = $("<a href='#" + path + json + "'><li>" + json + "</li></a>")
         .on('click', function () {
            if (!$('#' + path + json).is(':visible')) {
               $('#' + path + json).appendTo('content').fadeIn(fadetime);
            }
         });
      ;
   }
...
function mkentry(json, path = [], defer = []) {
   if (Array.isArray(json)) {
      json.forEach(function (e) { mkentry(e, path, defer) });
      ...
      defer.push($.get(
         ...
         function (data) {
            ...
            var entryHtml = $(
               '<entry id=' + path + json + ' style="display:none">'
               + '<h1>' + title + json + '<close></close></h1>'
               + marked(data)
               + '</entry>'
            ).appendTo('content');
            ...
            if (window.location.hash.substr(1) == entryHtml.prop('id')) {
